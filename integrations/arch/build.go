...
func Build(ctx context.Context, c *Config) error {
	...
	if err != nil {
		return fmt.Errorf("opening arch repo: %w", err)
	}
	...
	for _, rel := range releases {
		for _, asset := range rel.Assets {
			...
			if err != nil {
				return fmt.Errorf("downloading asset %q: %w", asset.Name, err)
			}
			if err := r.Add(asset.Name, data); err != nil {
				return fmt.Errorf("adding package %q: %w", asset.Name, err)
			}
			hasNew = true
		}
	}
	...
	if err := r.Write(); err != nil {
		return fmt.Errorf("writing arch dbs: %w", err)
	}

	if err := target.CopyFS(ctx, c.Target, os.DirFS(r.dir)); err != nil {
		return fmt.Errorf("publishing arch repo: %w", err)
	}
